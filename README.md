...
* absolute path - A path is said to be absolute if it provides the entire context
...
* relative path - A relative path is a way to locate a dir or file relative to
another directory. A relative path may not begin with a slash but may include .
and .. directories.

* file path - A file path ends with a filename and therefore may not end with a
slash.
