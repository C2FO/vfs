...
// Path returns the absolute file path including the file name.
func (f *File) Path() string {
	return utils.EnsureLeadingSlash(f.path)
}

// Authority implements [vfs.FileIdentity.Authority].
...
func (f *File) Stat() (fs.FileInfo, error) {
	...
	return ftpFileInfo{entry: entry, name: f.Name()}, nil
}
...
func (i ftpFileInfo) Mode() fs.FileMode  { return 0644 }
func (i ftpFileInfo) ModTime() time.Time { return i.entry.Time }
func (i ftpFileInfo) IsDir() bool        { return false }
...
func (f *File) Touch() error {
	...
	nf, err := f.Location().NewFile(tempFileNameGetter(f.Name()))
	if err != nil {
		return utils.WrapTouchError(err)
	}
	newFile, ok := nf.(*File)
	if !ok {
		return utils.WrapTouchError(fmt.Errorf("ftp: unexpected file type %T", nf))
	}

	if err := f.MoveToFile(newFile); err != nil {
		return utils.WrapTouchError(err)
	}
...
func (f *File) CopyToFile(file vfs.File) (err error) { //nolint:gocyclo
	...
	if f.offset != 0 {
		return utils.WrapCopyToFileError(vfsv7.ErrCopyToNotPossible)
	}
...
func (f *File) deleteObject(_ ...vfsopt.DeleteOption) error {
	dc, err := f.location.fileSystem.DataConn(context.TODO(), f.Location().Authority(), types.SingleOp, f)
	if err != nil {
