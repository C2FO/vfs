...
func (ts *fileTestSuite) TestRead() {
	...
	ftpfile.location.fileSystem.dataConnGetter = func(
		_ context.Context, _ authority.Authority, _ *FileSystem, _ *File, _ types.OpenType,
	) (types.DataConn, error) {
		return nil, dconnErr
	}
	...
	ftpfile.location.fileSystem.dataConnGetter = nil
}
...
func (ts *fileTestSuite) TestWrite() {
	...
	// get client error
	dconnErr := errors.New("some getDataConn error")
	file.location.fileSystem.dataConnGetter = func(
		_ context.Context, _ authority.Authority, _ *FileSystem, _ *File, _ types.OpenType,
	) (types.DataConn, error) {
		return nil, dconnErr
	}
	...
	ts.Require().ErrorIs(err, dconnErr, "should be right kind of error")
	file.location.fileSystem.dataConnGetter = nil
}
...
func (ts *fileTestSuite) TestSeek() {
	...
	ftpfile.location.fileSystem.dataConnGetter = getFakeDataConn
	defer func() { ftpfile.location.fileSystem.dataConnGetter = nil }()
...
func (ts *fileTestSuite) TestExists_noMLST() {
	...
	// stat client error
	ftpfile.(*File).location.fileSystem.clientGetter = clientGetterReturnsError
...
func (ts *fileTestSuite) TestCopyToFile() {
	...
	sourceFile.location.fileSystem.dataConnGetter = getFakeDataConn
	...
	targetFile.location.fileSystem.dataConnGetter = getFakeDataConn
	defer func() {
		sourceFile.location.fileSystem.dataConnGetter = nil
		targetFile.location.fileSystem.dataConnGetter = nil
	}()
	...
	fakeReadErrConn := NewFakeDataConn(types.OpenRead)
	fakeReadErrConn.AssertReadErr(os.ErrNotExist)
	sourceFile.location.fileSystem.resetConn = false
	sourceFile.location.fileSystem.dataconn = fakeReadErrConn
	err = sourceFile.CopyToFile(targetFile)
...
func (ts *fileTestSuite) TestCopyToLocation() {
	...
	targetLocation.fileSystem.dataConnGetter = getFakeDataConn
	defer func() {
		sourceFile.location.fileSystem.dataConnGetter = nil
		targetLocation.fileSystem.dataConnGetter = nil
	}()
...
func (ts *fileTestSuite) TestMoveToFile_differentAuthority() {
	...
	// CopyToFile failure on MoveToFile (run before successful move deletes the source)
	readErr := errors.New("some read error")
	fakeReadFailConn := NewFakeDataConn(types.OpenRead)
	fakeReadFailConn.AssertReadErr(readErr)
	sourceFile.location.fileSystem.dataconn = fakeReadFailConn
	sourceFile.location.fileSystem.resetConn = false
...
func (ts *fileTestSuite) TestMoveToFile_sameAuthority() {
	...
	// FakeDataConn does not call the mock client; List expectations would not be hit.
	err = sourceFile.MoveToFile(targetFile)
	ts.Require().NoError(err, "Error shouldn't be returned from successful call to MoveToFile")
	...
	// get client failure
	sourceFile.location.fileSystem.clientGetter = clientGetterReturnsError
	sourceFile.location.fileSystem.dataConnGetter = nil
	...
	sourceFile.location.fileSystem.clientGetter = nil
...
func (ts *fileTestSuite) TestTouch_notExists() {
	...
	file.location.fileSystem.dataConnGetter = getFakeDataConn
	defer func() { file.location.fileSystem.dataConnGetter = nil }()
...
func (ts *fileTestSuite) TestDelete() {
	...
	mockFTPClient.EXPECT().
		...
		Return(nil).
		Once()
	ts.Require().NoError(testFile.deleteObject(), "successful delete should not return an error")
	...
	mockFTPClient.EXPECT().
		...
		Return(delErr).
		Once()
	err = testFile.deleteObject()
	ts.Require().Error(err, "failed delete should return an error")
	ts.Require().ErrorIs(err, delErr, "should be right kind of error")

	// failure getting client
	testFile.location.fileSystem.clientGetter = clientGetterReturnsError
...
func (ts *fileTestSuite) TestLastModified() {
	...
	// stat client error
	ts.testFile.(*File).location.fileSystem.clientGetter = clientGetterReturnsError
