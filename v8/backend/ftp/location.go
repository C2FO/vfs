...
// List calls FTP ReadDir to list all files in the location's path (see [vfs.ListOption]).
...
func (l *Location) DeleteFile(fileName string, opts ...vfsopt.DeleteOption) error {
	...
	ff, ok := file.(*File)
	if !ok {
		return fmt.Errorf("ftp: unexpected file implementation %T", file)
	}
	return ff.deleteObject(opts...)
}
