...
// locationWithNewFile is a test double that forces NewFile to return a prebuilt *File (with custom opener).
type locationWithNewFile struct {
	*Location
	file *File
}

func (l *locationWithNewFile) NewFile(rel string, opts ...vfsopt.NewFileOption) (vfs.File, error) {
	return l.file, nil
}
...
func (ts *fileTestSuite) TestCopyToLocation() {
	...
	targetFS := &FileSystem{sftpclient: targetClient, options: Options{}}
	parentLoc, err := targetFS.NewLocation(auth2.String(), utils.EnsureTrailingSlash(path.Dir("/some/path.txt")))
	ts.Require().NoError(err)
	pl := parentLoc.(*Location)
	...
	targetLoc := &locationWithNewFile{Location: pl, file: targetFile}

	// run tests
	newFile, err := sourceFile.CopyToLocation(targetLoc)
	ts.Require().NoError(err, "Error shouldn't be returned from successful call to CopyToFile")
...
func (ts *fileTestSuite) TestMoveToLocation() {
	...
	targetFS := &FileSystem{sftpclient: targetClient, options: Options{}}
	parentLoc, err := targetFS.NewLocation(auth2.String(), utils.EnsureTrailingSlash(path.Dir("/some/other/path.txt")))
	ts.Require().NoError(err)
	pl := parentLoc.(*Location)
	...
	// run tests
	newFile, err := sourceFile.MoveToLocation(targetLoc)
	ts.Require().NoError(err, "Error shouldn't be returned from successful call to CopyToFile")
...
func (ts *fileTestSuite) TestDelete() {
	...
	err := ts.testFile.(*File).deleteObject()
...
func (ts *fileTestSuite) TestSize() {
	...
	ts.sftpMock.EXPECT().Stat(ts.testFile.Path()).Return(mocks.NewFileInfo(ts.T()), errors.New("some error")).Once()
	size, err = ts.testFile.Size()
