...
// ClientGetter resolves an SFTP client and its underlying SSH connection.
type ClientGetter func(a authority.Authority, opts Options) (Client, io.Closer, error)
...
func (fs *FileSystem) ensureClientGetter() {
	if fs.clientGetter == nil {
		fs.clientGetter = func(a authority.Authority, opts Options) (Client, io.Closer, error) {
			return GetClient(a, opts)
		}
	}
}
...
func (fs *FileSystem) Client(a authority.Authority) (Client, error) {
	...
	if fs.sftpclient == nil || (reflect.ValueOf(fs.sftpclient).IsValid() && reflect.ValueOf(fs.sftpclient).IsNil()) {
		var err error
		fs.sftpclient, fs.sshConn, err = fs.clientGetter(a, fs.options)
		if err != nil {
			return nil, err
		}
	}
