...
func (s *LocationTestSuite) TestList() {
	...
	listing, err := testsuite.CollectList(context.Background(), l)
...
func (s *LocationTestSuite) TestListByPrefix() {
	...
	fs := NewFileSystem(WithClient(client))
	l, _ := fs.NewLocation("test-container", "/some/folder/")
	listing, err := testsuite.CollectList(context.Background(), l, vfs.WithPrefix("file"))
	s.Require().NoError(err)
	s.Len(listing, 2)
...
func (s *LocationTestSuite) TestListByRegex() {
	...
	listing, err := testsuite.CollectList(context.Background(), l, vfs.WithRegexp(regex))
	s.Require().NoError(err)
