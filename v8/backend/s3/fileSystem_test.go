...
func (ts *fileSystemTestSuite) TestNewFileSystem() {
	...
	// test with client
	cli := mocks.NewClient(ts.T())
	newFS = NewFileSystem(WithClient(cli))
	ts.NotNil(newFS, "Should return a new fileSystem for s3")
	ts.Equal(cli, newFS.client, "Should set client to mock")
}
...
func (ts *fileSystemTestSuite) TestName_Error() {
	ts.Equal(fsDisplayName, s3fs.Name(), "Name() is s3 fsDisplayName const")
}
