...
## [[v7.28.0](https://github.com/C2FO/vfs/releases/tag/v7.28.0)] - 2026-08-28
### Added
...
- CI: Confirmed the root cause of the `releasegen` `protected branch hook declined` failure: `main`'s branch protection `required_status_checks` has no bypass mechanism for non-admin identities. Repository admins bypass branch protection entirely (`enforce_admins` is disabled), which is why manual PR merges have always worked; the `releasegen-bot` app is only on the pull-request-bypass and push-restriction allowlists, and classic branch protection has no equivalent bypass for required status checks. Since `releasegen` pushes a brand-new commit directly to `main`, it can never satisfy "must have a successful/skipped/neutral status" for any required context, so the push is always rejected—regardless of which workflows are wired to which events.
- CI: Migrated `main`'s classic branch protection to a GitHub Ruleset (`main-protection`) with the same pull-request-review, code-owner-review, and required-status-check rules, plus an explicit bypass actor for the `releasegen-bot` app with `bypass_mode: always`. Unlike classic branch protection, rulesets support bypassing required status checks for a specific non-admin actor, so `releasegen`'s post-merge push to `main` can now succeed without weakening enforcement for any other contributor.
...
### Fixed
- `backend/testsuite`: The conformance suite's special-character filename move test now compares `File.Path()` (raw and identical across every backend) instead of `File.URI()`, which is percent-encoded on some backends (e.g. `sftp`, `ftp`) and raw on others. This lets `sftp` and `ftp` pass the same conformance suite as the object-store backends without changing any backend's URI behavior.

## [[v7.21.0](https://github.com/C2FO/vfs/releases/tag/v7.21.0)] - 2026-07-23
### Added
...
## [[v7.20.0](https://github.com/C2FO/vfs/releases/tag/v7.20.0)] - 2026-06-19
### Added
...
- Use open source releasegen container
### Security
...
### Security
- Updated `github.com/go-jose/go-jose/v4` to `v4.1.4` and `go.opentelemetry.io/otel/sdk` to `v1.43.0` to address Dependabot advisories.
