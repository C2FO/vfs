...
returns vfs.Filesystem interface so it would have to be cast as s3.Filesystem to
use the following:
...
CopyObject method if the target file is also on S3, otherwise uses io.Copy.
...
for copying files will be utilized, otherwise, standard io.Copy will be done to
the new file.
...
```
Name returns the name portion of the file's key property. IE: "file.txt" of
...
```
Path return the directory portion of the file's key. IE: "path/to" of
"s3://some/path/to/file.txt
...
```
Read implements the standard for io.Reader. For this to work with an s3 file, a
...
subsequent write. When f.Close() is called, the contents of the buffer are used
...
```
NewFileSystem intializer for fileSystem struct accepts aws-sdk s3iface.S3API
...
s3.fileSystem.Client has the appropriate permissions. Will receive false without
...
the prefix arg passed to the function. The resource considerations of List()
...
(backed by an s3.File). The filePath argument is expected to be a relative path
