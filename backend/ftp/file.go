...
func (f *File) Touch() error {
	...
	dc, err := f.Location().FileSystem().(*FileSystem).DataConn(f.ctx, f.Location().Authority(), types.SingleOp, f)
	if err != nil {
		return utils.WrapTouchError(err)
	}
...
func (f *File) Read(p []byte) (n int, err error) {
	dc, err := f.Location().FileSystem().(*FileSystem).DataConn(f.ctx, f.Location().Authority(), types.OpenRead, f)
	if err != nil {
...
func (f *File) Seek(offset int64, whence int) (int64, error) {
	...
	_, err = f.Location().FileSystem().(*FileSystem).DataConn(f.ctx, f.Location().Authority(), mode, f)
	if err != nil {
		return 0, utils.WrapSeekError(err)
	}
...
func (f *File) Write(data []byte) (res int, err error) {
	dc, err := f.Location().FileSystem().(*FileSystem).DataConn(f.ctx, f.Location().Authority(), types.OpenWrite, f)
	if err != nil {
