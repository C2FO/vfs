...
func (ts *fileTestSuite) TestClose() {
	...
	// error closing ftpfile — dataconn must be cleared even on error (prevents zombie reuse)
	myCloseErr := errors.New("some close error")
	dc.EXPECT().Close().Return(myCloseErr).Once()
	err = ftpfile.Close()
	ts.Require().Error(err, "close error expected")
	ts.Nil(ftpfile.location.fileSystem.dataconn, "dataconn must be nil after error-close")

	// success closing ftpfile
	dc2 := mocks.NewDataConn(ts.T())
	ftpfile.location.fileSystem.dataconn = dc2
	ftpfile.offset = 1234
	dc2.EXPECT().Close().Return(nil).Once()
	err = ftpfile.Close()
...
func (ts *fileTestSuite) TestClose_zombieDataconnCleared() {
	...
	dc := mocks.NewDataConn(ts.T())
	dc.EXPECT().Mode().Return(types.OpenWrite)
	pipeErr := errors.New("io: read/write on closed pipe")
	dc.EXPECT().Write(mock.Anything).Return(0, pipeErr).Once()
	dc.EXPECT().Close().Return(errors.New("connection reset by peer")).Once()
	...
	// A subsequent Write must not panic — swap in a fresh working dataconn to assert
	// Write succeeds on retry.
	freshDC := mocks.NewDataConn(ts.T())
	freshDC.EXPECT().Mode().Return(types.OpenWrite)
	freshDC.EXPECT().Write(mock.Anything).Return(5, nil).Once()
...
func (ts *fileTestSuite) TestCopyToFile() {
	...
	fakeWriteDataConn.EXPECT().Close().Return(closeErr).Once()
	err = sourceFile.CopyToFile(targetFile)
	ts.Require().Error(err, "error is expected")
...
func (ts *fileTestSuite) TestTouch_exists() {
	...
	client.EXPECT().
		...
		Return(nil).
		Once()
	dconn.EXPECT().Close().Return(nil).Once()
