...
import (
	...
	"github.com/c2fo/vfs/v3/backend/gs"
	_os "github.com/c2fo/vfs/v3/backend/os"
	"github.com/c2fo/vfs/v3/backend/s3"
...
func (s *vfsTestSuite) FileSystem(baseLoc vfs.Location) {
	...
	/*
		...
		"volume" my be an empty string for filesystems that don't have a volume or
	...
	*/
	...
	/*
		...
		* path must end in separator character
		* path is expected always be absolute and therefore must begin with a separator character
...
func (s *vfsTestSuite) Location(baseLoc vfs.Location) {
	...
	*/
	...
	/*
		...
		paths's for locations will always end with a separator character.

		Path() string
	*/

	/*
		Exists returns boolean if the location exists. Also returns an error if any.

		For some keystore filesystems, a prefix/folder can't exist if there isn't an object with a key that contains that
		"location", so checking if a given "location" exists is effectively checking if keys with a particular prefix exist.
	...
	*/
	...
	/*
				...
				relativePath may use dot (. and ..) paths and may not begin with a separator character but may or may not end with
	...
	*/
	...
	/*
		...
		URI's for locations will always end with a separator character.
	...
	for _, testLoc := range s.testLocations {
		...
		*/
		...
		/*
			...
			the contents will be overwritten with the current file's contents. If the location does not exist, an attempt will
			be made to create it.  In the case of an error, nil is returned for the file.
