...
func (s *memFileTest) TestRARO() {
	...
	s.Require().NoError(s.testFile.Close(), "unexpected error closing file") // close the file so the buffer is stored into "contents"
	...
	s.Require().Error(err, "read was expected to fail")
}
...
func (s *memFileTest) TestRARC() {
	...
	s.Require().NoError(err, "read after read failed!")
...
func (s *memFileTest) TestNewFile() {
	...
	s.Require().NoError(err, "unexpected error retrieving file")
...
func (s *memFileTest) TestSeek2() {
	...
	s.Require().NoError(err, "seeking beyond EOF should be allowed")
	// trying to read should return EOF since we're beyond the data
	_, err = newFile.Read(make([]byte, 1))
	s.Require().ErrorIs(err, io.EOF, "expected EOF when reading beyond data")
	...
	s.Require().Error(err, "seek error expected for seeking into negative bounds")
	...
	s.Require().ErrorIs(err, vfs.ErrSeekInvalidWhence, "expected Seek error")
...
func (s *memFileTest) TestNameToURI() {
	...
	s.Require().NoError(returnedFile.Touch(), "unexpected error touching file")
...
func (s *memFileTest) TestSeek() {
	...
	s.Require().NoError(file.Close(), "close error not expected")
	...
	s.Require().NoError(file.Delete(), "file was not deleted properly")
...
func (s *memFileTest) TestCopyToLocationOW() {
	...
	s.NotNil(copiedFile)
	s.Require().NoError(copiedFile.Close(), "unexpected close error")
...
func (s *memFileTest) TestCopyToNilFile() {
	...
	s.Require().Error(err, "expected error creating file (bad path)")
	err = s.testFile.CopyToFile(newFile)
	s.Require().Error(err, "expected error for copying to nil file")
}
...
func (s *memFileTest) TestCopyToFile() {
	...
	s.Require().NoError(err, "copy to file failed unexpectedly")
...
func (s *memFileTest) TestCopyToFileOS() {
	...
	s.Require().NoError(err, "copy to file failed unexpectedly")
	s.Require().NoError(osFile.Close(), "unexpected error closing a file")

	size1, err := s.testFile.Size()
	s.Require().NoError(err, "unexpected error retrieving size")
...
func (s *memFileTest) TestEmptyCopyToFile() {
	...
	s.Require().NoError(emptyFile.Close(), "unexpected close error")
	// call to CopyToFile
...
func (s *memFileTest) TestMoveToLocation() {
	...
	s.Require().NoError(err, "unexpected move to location error")
...
func (s *memFileTest) TestMoveToFile() {
	...
	s.Require().NoError(err, "Write failed unexpectedly")
	s.Require().NoError(s.testFile.Close(), "unexpected close error")
	...
	s.Require().NoError(err, "move to file failed")
	...
	s.Require().NoError(err, "read unexpectedly failed")
...
func (s *memFileTest) TestRead() {
	...
	s.Require().NoError(err, "unexpected new file error")
	num, err := fileToRead.Write(expectedSlice)
	s.Require().NoError(err, "unexpected write error")
	s.Equal(length, num)
	s.Require().NoError(fileToRead.Close(), "close error not expected")
	...
	s.Require().ErrorIs(err, fs.ErrNotExist, "expected Read error")
}
...
func (s *memFileTest) TestWriteThenReadNoClose() {
	...
	s.Require().NoError(file.Delete(), "unexpected error deleting file")
	found2, err := file.Exists()
	s.Require().NoError(err, "exists error not expected")
	s.False(found2)
}
...
func (s *memFileTest) TestPath() {
	...
	s.Require().NoError(err, "unexpected error creating a new file")
	s.Require().NoError(file1.Touch(), "unexpected error touching file")
...
func (s *memFileTest) TestSeekBeyondEOF() {
	...
	s.Require().NoError(err, "writing after seeking beyond EOF should work")
	s.Require().NoError(file.Close(), "unexpected close error")

	// Verify file size
	size, err := file.Size()
	s.Require().NoError(err, "unexpected size error")
